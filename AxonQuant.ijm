...
// Set pre-crush norm and intensity threshold from user input
pcMx = getNumber("Set Maximum Pre-Crush Intensity", 0);
print("Maximum Pre-Crush Intensity: " + pcMx);
threshold = getNumber("Set Threshold Intensity", 0);
...
setResult("", nSlices, "Average ROI Intensity");
for (i = 0; i < nROIs; i++)
...
setResult("", nSlices + 2, "Average ROI Intensity / ROI Length (Nerve Diameter)");
for (i = 0; i < nROIs; i++)
    setResult(roiLabels[i], nSlices + 2, (total[i] / nSlices) / len[i]);
setResult("", nSlices + 3, "Axon Density Relative to Pre-Crush");
